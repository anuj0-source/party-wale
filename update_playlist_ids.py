...
# Some fallback ID if scrape fails
FALLBACK_ID = 'K4DyBUG242c'

def get_yt_id(title, artist):
    # Searching for "lyrical" to avoid official videos that might be embed-disabled
    query = urllib.parse.quote(f"{title} {artist} lyrical")
    ...
    # Try one more time with just the title
    time.sleep(0.5)
...
print("Fetching YouTube IDs for 200 songs... This will take a minute or two.")
results = []
# Using ThreadPool to speed it up (but not too high to avoid getting blocked)
with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
    futures = {executor.submit(get_yt_id, t, a): (t, a) for t, a in songs}
    
    for future in concurrent.futures.as_completed(futures):
        title, artist = futures[future]
        try:
            yid = future.result()
            results.append((title, artist, yid))
            print(f"[{len(results)}/200] {title} -> {yid}")
        except Exception as exc:
            results.append((title, artist, FALLBACK_ID))
            print(f"[{len(results)}/200] {title} generated an exception: {exc}")
...
for i, (title, artist) in enumerate(songs):
    yid = FALLBACK_ID
    for r_title, r_artist, r_yid in results:
        if r_title == title and r_artist == artist:
            yid = r_yid
            break
...
print("Playlist generated successfully with 200 real YouTube IDs.")
